function [A, W] = fpica(X, whiteningMatrix, dewhiteningMatrix, approach, ...
			...
			s_verbose, nList, numRandomStarts);
...
% nList									:list of indices for hf based fn
...
if nargin < 22, numRandomStarts = 1; end
if nargin < 21, nList = 0:10; end
...
if approachMode == 2
  ...
  while round <= numOfIC,
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% swarm optimization
	wRound = randn (vectorSize, 1);
    wRound = wRound - BSwarm * BSwarm' * wRound;
    wRound = wRound / norm(wRound);
    randomStart = 1;
  
  while randomStart < 1+numRandomStarts
	B = BSwarm;
	myy = myyOrig;
    usedNlinearity = gOrig;
    stroke = 0;
	...
	% hfpow
	   case 50
	y = w'*X;
	w = w + X*firstDerivativeHFPow(y, nList)'/ ...
        (numSamples*abs(mean(secondDerivativeHFPow(y, nList))));
	   case 51
	y = w'*X;
	w = w + myy*X*firstDerivativeHFPow(y, nList)'/ ...
        (numSamples*abs(mean(secondDerivativeHFPow(y, nList))));
	   case 52
	Xsub = X(:,getSamples(numSamples, sampleSize));
	y = w'*Xsub;
	w = w + Xsub*firstDerivativeHFPow(y, nList)'/(numSamples*abs(mean(secondDerivativeHFPow(y, nList))));
	   case 53
	Xsub = X(:,getSamples(numSamples, sampleSize));
	y = w'*Xsub;
	w = w + myy*Xsub*firstDerivativeHFPow(y, nList)'/(numSamples*abs(mean(secondDerivativeHFPow(y, nList))));
    
	%hfpowl
	   case 60
	y = w'*X;
   	lr = (maxNumIterations-i+1)/maxNumIterations;
%     lr = 0.5^floor((1+i)/(0.1*maxNumIterations));
	w = w + lr*X*firstDerivativeHFPow(y, nList)'/ ...
        (numSamples*(mean(secondDerivativeHFPow(y, nList))));
	   case 61
	y = w'*X;
	lr = (maxNumIterations-i+1)/maxNumIterations;
%     lr = 0.5^floor((1+i)/(0.1*maxNumIterations));
	w = w + myy*lr*X*firstDerivativeHFPow(y, nList)'/ ...
        (numSamples*(mean(secondDerivativeHFPow(y, nList))));
	   case 62
	Xsub = X(:,getSamples(numSamples, sampleSize));
	y = w'*Xsub;
	lr = (maxNumIterations-i+1)/maxNumIterations;
%     lr = 0.5^floor((1+i)/(0.1*maxNumIterations));
	w = w + lr*Xsub*firstDerivativeHFPow(y, nList)'/(numSamples*abs(mean(secondDerivativeHFPow(y, nList))));
	   case 63
	Xsub = X(:,getSamples(numSamples, sampleSize));
	y = w'*Xsub;
	lr = (maxNumIterations-i+1)/maxNumIterations;
%     lr = 0.5^floor((1+i)/(0.1*maxNumIterations));
	w = w + myy*lr*Xsub*firstDerivativeHFPow(y, nList)'/(numSamples*abs(mean(secondDerivativeHFPow(y, nList))));

	% hfsum
	   case 70
	y = w'*X;
	w = (X*firstDerivativeHFSum(y, nList)')/numSamples - ...
        mean(secondDerivativeHFSum(y, nList))*w;
       case 71
	y = w'*X;
	firstD = X*firstDerivativeHFSum(y, nList)';
	Beta = w'*firstD;
	w = w - myy*((firstD - Beta*w)/ ...
        (sum(secondDerivativeHFSum(y, nList))-Beta));
       case 72
	Xsub = X(:,getSamples(numSamples, sampleSize));
	y = w'*Xsub;
	w = (Xsub*firstDerivativeHFSum(y, nList)')/size(Xsub, 2) - ...
        mean(secondDerivativeHFSum(y, nList))*w;
	   case 73
	Xsub = X(:,getSamples(numSamples, sampleSize));
	y = w'*Xsub;
	firstD = Xsub*firstDerivativeHFSum(y, nList)';
	Beta = w'*firstD;
	w = w - myy*((firstD - Beta*w)/(sum(secondDerivativeHFSum(y, nList))-Beta));
	...
	lr = (maxNumIterations-i+1)/maxNumIterations;
%     lr = 0.5^floor((1+i)/(0.1*maxNumIterations));
	w = lr*(X*firstDerivativeHFSum(y, nList)')/numSamples - ...
        mean(secondDerivativeHFSum(y, nList))*w;
       case 81
	y = w'*X;
	firstD = X*firstDerivativeHFSum(y, nList)';
	Beta = w'*firstD;
	lr = myy*(maxNumIterations-i+1)/maxNumIterations;
%     lr = 0.5^floor((1+i)/(0.1*maxNumIterations));
	w = w - lr*((firstD - Beta*w)/(sum(secondDerivativeHFSum(y, nList))-Beta));
	   case 82
	Xsub = X(:,getSamples(numSamples, sampleSize));
	y = w'*Xsub;
	lr = (maxNumIterations-i+1)/maxNumIterations;
%     lr = 0.5^floor((1+i)/(0.1*maxNumIterations));
	w = lr*(Xsub*firstDerivativeHFSum(y, nList)')/size(Xsub, 2) - mean(secondDerivativeHFSum(y, nList))*w;
	   case 83
	Xsub = X(:,getSamples(numSamples, sampleSize));
	y = w'*Xsub;
	firstD = Xsub*firstDerivativeHFSum(y, nList)';
...
%     lr = 0.5^floor((1+i)/(0.1*maxNumIterations));
	...
	% swarm optimization
	if i < maxNumIterations + 1
		gOrig10 = floor(gOrig/10);
		yOld = wRound' * X;
		y = w' * X;
		switch gOrig10
			case 1
				if abs(fPow3(y)) > abs(fPow3(yOld)), wRound = w; end
			case 2
				if abs(fTanh(y)) > abs(fTanh(yOld)), wRound = w; end
			case 3
				if abs(fGaus(y)) > abs(fGaus(yOld)), wRound = w; end
			case 4
				if abs(fSkew(y)) > abs(fSkew(yOld)), wRound = w; end
			case 5
				if abs(estimateHFPower(y, nList)) > abs(estimateHFPower(yOld, nList))
					wRound = w;
				end
			case 6
				if abs(estimateHFPower(y, nList)) > abs(estimateHFPower(yOld, nList))
			...
			case 7
				if abs(estimateHFSum(y, nList)) > abs(estimateHFSum(yOld, nList))
					wRound = w;
				end
			case 8
				if abs(estimateHFSum(y, nList)) > abs(estimateHFSum(yOld, nList))
	...
	% Calculate the de-whitened vector.
	ASwarm(:, round) = dewhiteningMatrix * wRound;
	% Calculate ICA filter.
	WSwarm(round, :) = wRound' * whiteningMatrix;
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	round = round + 1;
  end
